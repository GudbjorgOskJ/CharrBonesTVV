...
####Within Lower Jawgdf.art$id
#May need to change to 
missing<-gdf.art$id[which(!(gdf.art$id %in% gdf.den$id))]
#PC3846 missing from dentary csv, so removed here from articular angular gdf and quadrate gdf
dimnames(gdf.art$coords)[[3]][49]  
gdf.art$coords<-gdf.art$coords[,,-49]
gdf.art$Csize<-gdf.art$Csize[-49]
gdf.art$morph<-gdf.art$morph[-49]
gdf.art$csize<-gdf.art$csize[-49]
gdf.art$logcsize<-gdf.art$logcsize[-49]
gdf.art$ind<-gdf.art$ind[-49]
gdf.art$length<-gdf.art$length[-49]
gdf.art$weight<-gdf.art$weight[-49]
gdf.art$id<-gdf.art$id[-49]
gdf.art$sex<-gdf.art$sex[-49]
gdf.art$age<-gdf.art$age[-49]
gdf.art$loglength<-gdf.art$loglength[-49]


dimnames(gdf.qu$coords)[[3]][49]  
gdf.qu$coords<-gdf.qu$coords[,,-49]
gdf.qu$Csize<-gdf.qu$Csize[-49]
gdf.qu$morph<-gdf.qu$morph[-49]
gdf.qu$csize<-gdf.qu$csize[-49]
gdf.qu$logcsize<-gdf.qu$logcsize[-49]
gdf.qu$ind<-gdf.qu$ind[-49]
gdf.qu$length<-gdf.qu$length[-49]
gdf.qu$weight<-gdf.qu$weight[-49]
gdf.qu$id<-gdf.qu$id[-49]
gdf.qu$sex<-gdf.qu$sex[-49]
gdf.qu$age<-gdf.qu$age[-49]
gdf.qu$loglength<-gdf.qu$loglength[-49]


names3<-gsub("_01","",dimnames(gdf.den$coords)[[3]])
names3<-gsub("_02","",names3)
names3<-gsub("_03","",names3)
dimnames(gdf.den$coords)[[3]]<-names3
names(gdf.den$Csize)<-names3
names(gdf.den$logcsize)<-names3

dimnames(gdf.art$coords)[[3]]<-names3
names(gdf.art$Csize)<-names3
names(gdf.art$logcsize)<-names3

dimnames(gdf.qu$coords)[[3]]<-names3
names(gdf.qu$Csize)<-names3
names(gdf.qu$logcsize)<-names3

PLS1<-two.b.pls(gdf.den$coords, gdf.art$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS1)
plot(PLS1)

PLS2<-two.b.pls(gdf.den$coords, gdf.qu$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS2)
plot(PLS2)

PLS3<-two.b.pls(gdf.art$coords, gdf.qu$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS3)
plot(PLS3)

####Within Upper Jaw
missing<-gdf.pre$id[which(!(gdf.pre$id %in% gdf.max$id))]
#PC4224 missing from maxilla csv, so removed here from premaxilla gdf 

dimnames(gdf.pre$coords)[[3]][96]  
gdf.pre$coords<-gdf.pre$coords[,,-96]
gdf.pre$Csize<-gdf.pre$Csize[-96]
gdf.pre$morph<-gdf.pre$morph[-96]
gdf.pre$csize<-gdf.pre$csize[-96]
gdf.pre$logcsize<-gdf.pre$logcsize[-96]
gdf.pre$ind<-gdf.pre$ind[-96]
gdf.pre$length<-gdf.pre$length[-96]
gdf.pre$weight<-gdf.pre$weight[-96]
gdf.pre$id<-gdf.pre$id[-96]
gdf.pre$sex<-gdf.pre$sex[-96]
gdf.pre$age<-gdf.pre$age[-96]
gdf.pre$loglength<-gdf.pre$loglength[-96]

names4<-gsub("_01","",dimnames(gdf.max$coords)[[3]])
names4<-gsub("_02","",names4)
names4<-gsub("_03","",names4)
dimnames(gdf.max$coords)[[3]]<-names4
names(gdf.max$Csize)<-names4
names(gdf.max$logcsize)<-names4

dimnames(gdf.pre$coords)[[3]]<-names4
names(gdf.pre$Csize)<-names4
names(gdf.pre$logcsize)<-names4


PLS4<-two.b.pls(gdf.pre$coords, gdf.max$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS4)
plot(PLS4)

#many missing from supramaxilla, so individuals now removed from premaxilla and maxilla gdf ("PC3928" "PC3930" "PC3933" "PC4207" "PC4209" "PC4215" "PC4261")
missing<-gdf.pre$id[which(!(gdf.pre$id %in% gdf.sup$id))]
missnum<-c(26,28,31,79,81,87,131)

dimnames(gdf.pre$coords)[[3]][missnum]
gdf.pre$coords<-gdf.pre$coords[,,-missnum]
gdf.pre$Csize<-gdf.pre$Csize[-missnum]
gdf.pre$morph<-gdf.pre$morph[-missnum]
gdf.pre$csize<-gdf.pre$csize[-missnum]
gdf.pre$logcsize<-gdf.pre$logcsize[-missnum]
gdf.pre$ind<-gdf.pre$ind[-missnum]
gdf.pre$length<-gdf.pre$length[-missnum]
gdf.pre$weight<-gdf.pre$weight[-missnum]
gdf.pre$id<-gdf.pre$id[-missnum]
gdf.pre$sex<-gdf.pre$sex[-missnum]
gdf.pre$age<-gdf.pre$age[-missnum]
gdf.pre$loglength<-gdf.pre$loglength[-missnum]

dimnames(gdf.max$coords)[[3]][missnum]
gdf.max$coords<-gdf.max$coords[,,-missnum]
gdf.max$Csize<-gdf.max$Csize[-missnum]
gdf.max$morph<-gdf.max$morph[-missnum]
gdf.max$csize<-gdf.max$csize[-missnum]
gdf.max$logcsize<-gdf.max$logcsize[-missnum]
gdf.max$ind<-gdf.max$ind[-missnum]
gdf.max$length<-gdf.max$length[-missnum]
gdf.max$weight<-gdf.max$weight[-missnum]
gdf.max$id<-gdf.max$id[-missnum]
gdf.max$sex<-gdf.max$sex[-missnum]
gdf.max$age<-gdf.max$age[-missnum]
gdf.max$loglength<-gdf.max$loglength[-missnum]

names5<-gsub("_01","",dimnames(gdf.sup$coords)[[3]])
names5<-gsub("_02","",names5)
names5<-gsub("_03","",names5)
dimnames(gdf.sup$coords)[[3]]<-names5
names(gdf.sup$Csize)<-names5
names(gdf.sup$logcsize)<-names5


PLS5<-two.b.pls(gdf.pre$coords, gdf.sup$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS5)
plot(PLS5)

PLS6<-two.b.pls(gdf.max$coords, gdf.sup$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS6)
plot(PLS6)



####Across Upper and Lower Jaw
missing<-gdf.sup$id[which(!(gdf.sup$id %in% gdf.den$id))]
#PC3846 missing from dentary csv, so removed here from premaxilla gdf and maxilla gdf and supramaxilla gdf

dimnames(gdf.pre$coords)[[3]][46]  
gdf.pre$coords<-gdf.pre$coords[,,-46]
gdf.pre$Csize<-gdf.pre$Csize[-46]
gdf.pre$morph<-gdf.pre$morph[-46]
gdf.pre$csize<-gdf.pre$csize[-46]
gdf.pre$logcsize<-gdf.pre$logcsize[-46]
gdf.pre$ind<-gdf.pre$ind[-46]
gdf.pre$length<-gdf.pre$length[-46]
gdf.pre$weight<-gdf.pre$weight[-46]
gdf.pre$id<-gdf.pre$id[-46]
gdf.pre$sex<-gdf.pre$sex[-46]
gdf.pre$age<-gdf.pre$age[-46]
gdf.pre$loglength<-gdf.pre$loglength[-46]

dimnames(gdf.max$coords)[[3]][46]  
gdf.max$coords<-gdf.max$coords[,,-46]
gdf.max$Csize<-gdf.max$Csize[-46]
gdf.max$morph<-gdf.max$morph[-46]
gdf.max$csize<-gdf.max$csize[-46]
gdf.max$logcsize<-gdf.max$logcsize[-46]
gdf.max$ind<-gdf.max$ind[-46]
gdf.max$length<-gdf.max$length[-46]
gdf.max$weight<-gdf.max$weight[-46]
gdf.max$id<-gdf.max$id[-46]
gdf.max$sex<-gdf.max$sex[-46]
gdf.max$age<-gdf.max$age[-46]
gdf.max$loglength<-gdf.max$loglength[-46]

dimnames(gdf.sup$coords)[[3]][46]  
gdf.sup$coords<-gdf.sup$coords[,,-46]
gdf.sup$Csize<-gdf.sup$Csize[-46]
gdf.sup$morph<-gdf.sup$morph[-46]
gdf.sup$csize<-gdf.sup$csize[-46]
gdf.sup$logcsize<-gdf.sup$logcsize[-46]
gdf.sup$ind<-gdf.sup$ind[-46]
gdf.sup$length<-gdf.sup$length[-46]
gdf.sup$weight<-gdf.sup$weight[-46]
gdf.sup$id<-gdf.sup$id[-46]
gdf.sup$sex<-gdf.sup$sex[-46]
gdf.sup$age<-gdf.sup$age[-46]
gdf.sup$loglength<-gdf.sup$loglength[-46]


#many missing from supramaxilla, so individuals now removed from premaxilla and maxilla gdf ("PC3928" "PC3930" "PC3933" "PC4207" "PC4209" "PC4215" "PC4261")
missing<-gdf.den$id[which(!(gdf.den$id %in% gdf.sup$id))]
missnum<-c(26,28,31,78,80,86,131)

dimnames(gdf.den$coords)[[3]][missnum]
gdf.den$coords<-gdf.den$coords[,,-missnum]
gdf.den$Csize<-gdf.den$Csize[-missnum]
gdf.den$morph<-gdf.den$morph[-missnum]
gdf.den$csize<-gdf.den$csize[-missnum]
gdf.den$logcsize<-gdf.den$logcsize[-missnum]
gdf.den$ind<-gdf.den$ind[-missnum]
gdf.den$length<-gdf.den$length[-missnum]
gdf.den$weight<-gdf.den$weight[-missnum]
gdf.den$id<-gdf.den$id[-missnum]
gdf.den$sex<-gdf.den$sex[-missnum]
gdf.den$age<-gdf.den$age[-missnum]
gdf.den$loglength<-gdf.den$loglength[-missnum]


dimnames(gdf.art$coords)[[3]][missnum]
gdf.art$coords<-gdf.art$coords[,,-missnum]
gdf.art$Csize<-gdf.art$Csize[-missnum]
gdf.art$morph<-gdf.art$morph[-missnum]
gdf.art$csize<-gdf.art$csize[-missnum]
gdf.art$logcsize<-gdf.art$logcsize[-missnum]
gdf.art$ind<-gdf.art$ind[-missnum]
gdf.art$length<-gdf.art$length[-missnum]
gdf.art$weight<-gdf.art$weight[-missnum]
gdf.art$id<-gdf.art$id[-missnum]
gdf.art$sex<-gdf.art$sex[-missnum]
gdf.art$age<-gdf.art$age[-missnum]
gdf.art$loglength<-gdf.art$loglength[-missnum]

dimnames(gdf.qu$coords)[[3]][missnum]
gdf.qu$coords<-gdf.qu$coords[,,-missnum]
gdf.qu$Csize<-gdf.qu$Csize[-missnum]
gdf.qu$morph<-gdf.qu$morph[-missnum]
gdf.qu$csize<-gdf.qu$csize[-missnum]
gdf.qu$logcsize<-gdf.qu$logcsize[-missnum]
gdf.qu$ind<-gdf.qu$ind[-missnum]
gdf.qu$length<-gdf.qu$length[-missnum]
gdf.qu$weight<-gdf.qu$weight[-missnum]
gdf.qu$id<-gdf.qu$id[-missnum]
gdf.qu$sex<-gdf.qu$sex[-missnum]
gdf.qu$age<-gdf.qu$age[-missnum]
gdf.qu$loglength<-gdf.qu$loglength[-missnum]


missing<-gdf.qu$id[which(!(gdf.qu$id %in% gdf.max$id))]
#PC4224 missing from maxilla csv, so removed here from dentary, articular, quadrate gdf  

dimnames(gdf.den$coords)[[3]][89]
gdf.den$coords<-gdf.den$coords[,,-89]
gdf.den$Csize<-gdf.den$Csize[-89]
gdf.den$morph<-gdf.den$morph[-89]
gdf.den$csize<-gdf.den$csize[-89]
gdf.den$logcsize<-gdf.den$logcsize[-89]
gdf.den$ind<-gdf.den$ind[-89]
gdf.den$length<-gdf.den$length[-89]
gdf.den$weight<-gdf.den$weight[-89]
gdf.den$id<-gdf.den$id[-89]
gdf.den$sex<-gdf.den$sex[-89]
gdf.den$age<-gdf.den$age[-89]
gdf.den$loglength<-gdf.den$loglength[-89]

dimnames(gdf.art$coords)[[3]][89]
gdf.art$coords<-gdf.art$coords[,,-89]
gdf.art$Csize<-gdf.art$Csize[-89]
gdf.art$morph<-gdf.art$morph[-89]
gdf.art$csize<-gdf.art$csize[-89]
gdf.art$logcsize<-gdf.art$logcsize[-89]
gdf.art$ind<-gdf.art$ind[-89]
gdf.art$length<-gdf.art$length[-89]
gdf.art$weight<-gdf.art$weight[-89]
gdf.art$id<-gdf.art$id[-89]
gdf.art$sex<-gdf.art$sex[-89]
gdf.art$age<-gdf.art$age[-89]
gdf.art$loglength<-gdf.art$loglength[-89]


dimnames(gdf.qu$coords)[[3]][89]
gdf.qu$coords<-gdf.qu$coords[,,-89]
gdf.qu$Csize<-gdf.qu$Csize[-89]
gdf.qu$morph<-gdf.qu$morph[-89]
gdf.qu$csize<-gdf.qu$csize[-89]
gdf.qu$logcsize<-gdf.qu$logcsize[-89]
gdf.qu$ind<-gdf.qu$ind[-89]
gdf.qu$length<-gdf.qu$length[-89]
gdf.qu$weight<-gdf.qu$weight[-89]
gdf.qu$id<-gdf.qu$id[-89]
gdf.qu$sex<-gdf.qu$sex[-89]
gdf.qu$age<-gdf.qu$age[-89]
gdf.qu$loglength<-gdf.qu$loglength[-89]

names6<-gsub("_01","",dimnames(gdf.den$coords)[[3]])
names6<-gsub("_02","",names6)
names6<-gsub("_03","",names6)
dimnames(gdf.den$coords)[[3]]<-names6
names(gdf.den$Csize)<-names6
names(gdf.den$logcsize)<-names6

dimnames(gdf.art$coords)[[3]]<-names6
names(gdf.art$Csize)<-names6
names(gdf.art$logcsize)<-names6

dimnames(gdf.qu$coords)[[3]]<-names6
names(gdf.qu$Csize)<-names6
names(gdf.qu$logcsize)<-names6

dimnames(gdf.pre$coords)[[3]]<-names6
names(gdf.pre$Csize)<-names6
names(gdf.pre$logcsize)<-names6

dimnames(gdf.max$coords)[[3]]<-names6
names(gdf.max$Csize)<-names6
names(gdf.max$logcsize)<-names6

dimnames(gdf.sup$coords)[[3]]<-names6
names(gdf.sup$Csize)<-names6
names(gdf.sup$logcsize)<-names6



PLS7<-two.b.pls(gdf.den$coords, gdf.pre$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS7)
plot(PLS7)

PLS8<-two.b.pls(gdf.art$coords, gdf.pre$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS8)
plot(PLS8)

PLS9<-two.b.pls(gdf.qu$coords, gdf.pre$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS9)
plot(PLS9)

PLS10<-two.b.pls(gdf.den$coords, gdf.max$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS10)
plot(PLS10)

PLS11<-two.b.pls(gdf.art$coords, gdf.max$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS11)
plot(PLS11)

PLS12<-two.b.pls(gdf.qu$coords, gdf.max$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS12)
plot(PLS12)

PLS13<-two.b.pls(gdf.den$coords, gdf.sup$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS13)
plot(PLS13)

PLS14<-two.b.pls(gdf.art$coords, gdf.sup$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS14)
plot(PLS14)

PLS15<-two.b.pls(gdf.qu$coords, gdf.sup$coords, iter = 999, seed = NULL, print.progress = TRUE)
summary(PLS15)
plot(PLS15)


#########Compare PLS (lower jaw)

lowerPLS<-compare.pls(PLS1,PLS2,PLS3)
summary(lowerPLS)


#########Compare PLS (Upper jaw)

UpperPLS<-compare.pls(PLS4,PLS5,PLS6)
summary(UpperPLS)


#########Compare Across Jaws

AcrossPLS<-compare.pls(PLS7,PLS8,PLS9,PLS10,PLS11,PLS12,PLS13,PLS14,PLS15)
summary(AcrossPLS)

###############Morph effect test

#dentary-Articular
group<-factor(gdf.art$morph)
levels(group)

dentary.coords<-gdf.den$coords
articular.coords<-gdf.art$coords

den.coords.gp <- coords.subset(dentary.coords, group)
art.coords.gp <- coords.subset(articular.coords, group)

integ.tests <- Map(function(x,y) integration.test(x, y, iter=499, 
                                                  print.progress = FALSE), den.coords.gp, art.coords.gp)
...
dentary.coords<-gdf.den$coords
premaxilla.coords<-gdf.pre$coords

den.coords.gp <- coords.subset(dentary.coords, group)
pre.coords.gp <- coords.subset(premaxilla.coords, group)

integ.tests <- Map(function(x,y) integration.test(x, y, iter=499, 
                                                  print.progress = FALSE), den.coords.gp, pre.coords.gp)
...
dentary.coords<-gdf.den$coords
maxilla.coords<-gdf.max$coords

den.coords.gp <- coords.subset(dentary.coords, group)
max.coords.gp <- coords.subset(maxilla.coords, group)

integ.tests <- Map(function(x,y) integration.test(x, y, iter=499, 
                                                  print.progress = FALSE), den.coords.gp, max.coords.gp)
...
integ.tests <- Map(function(x,y) integration.test(x, y, iter=499, 
                                                  print.progress = FALSE), pre.coords.gp, max.coords.gp)
