...
##If excel
#datallfish<-read_excel("Master_sheetGOJ.xlsx", na="NA")
#datallfish$date_net_set<-as.factor(datallfish$date_net_set)
#datallfish$date_net_up<-as.factor(datallfish$date_net_up)
#datallfish$morph<-as.factor(datallfish$morph)
#datallfish$sex<-as.factor(datallfish$sex)
...
###By morph###
m3<-ggplot(datallfish, aes(x=age, fill=morph, col=morph)) +  geom_bar(alpha=0.5, position="identity")+ labs(x="Age", y="Count", fill="Morph")+  guides(fill = "none")+ facet_grid(morph ~ .) + guides(col = "none") + scale_color_manual(values=c("#00BA38","magenta4","#F8766D","#00BFC4")) + scale_fill_manual(values = c("#00BA38","magenta4","#F8766D","#00BFC4")) + theme_bw()
ml<-ggplot(datallfish, aes(y= length_cm, x = morph, fill = morph)) + geom_boxplot() + labs(y="Length (cm)", x="Morph", fill="Morph") + guides(fill = "none")+ scale_fill_manual(values=c("#00BA38","magenta4","#F8766D","#00BFC4")) + coord_flip() + geom_jitter(width=0.1,alpha=0.3)+ theme_bw()+ scale_x_discrete(limits=c("SB", "PL", "PI", "LB"))
mw<-ggplot(datallfish, aes(y= log(weight_g), x = morph, fill = morph)) + geom_boxplot() + labs(y="Log Weight (g)", x="Morph", fill="Morph")+ guides(fill = "none") + scale_fill_manual(values=c("#00BA38","magenta4","#F8766D","#00BFC4")) + coord_flip() + geom_jitter(width=0.1,alpha=0.3)+ theme_bw()+ scale_x_discrete(limits=c("SB", "PL", "PI", "LB"))
...
mmsa<-ggplot(datallfish, aes(x=age, fill=morph, col=morph)) +  geom_bar(alpha=0.5, position="identity")+ labs(x="Age", y="Count", fill="Morph")+  guides(fill = "none")+ facet_grid(morph ~ sex) + guides(col = "none") + scale_color_manual(values=c("#00BA38","magenta4","#F8766D","#00BFC4")) + scale_fill_manual(values = c("#00BA38","magenta4","#F8766D","#00BFC4")) + theme_bw()
mmsl<-ggplot(datallfish, aes(y= length_cm, x = morph, fill = sex)) + geom_boxplot() + labs(y="Length (cm)", x="Morph", fill="Sex") + guides(fill = "none")+ scale_fill_manual(values=c("#FF0066", "#3300FF")) + coord_flip() + geom_jitter(width=0.1,alpha=0.3)+ theme_bw()+ scale_x_discrete(limits=c("SB", "PL", "PI", "LB"))
mmsw<-ggplot(datallfish, aes(y= log(weight_g), x = morph, fill = sex)) + geom_boxplot() + labs(y="Log Weight (g)", x="Morph", fill="Sex")+ guides(fill = "none") + scale_fill_manual(values=c("#FF0066", "#3300FF")) + coord_flip() + geom_jitter(width=0.1,alpha=0.3)+ theme_bw()+ scale_x_discrete(limits=c("SB", "PL", "PI", "LB"))
